...
class Member:
    def __init__(self, codname, name, surname1, surname2, edicom_init_date, team_init_date):
        """edicom_init_date and team_init_date must be in format dd-mm-yyyy"""
        ...
        try:
            self.edicom_init_date = datetime.strptime(edicom_init_date, allowed_format)
        except ValueError:
            self.edicom_init_date = None
            print("error: edicom_init_date format is not valid. Registered as None.")
    ...
    def get_edicom_seniority(self):
        """Returns the seniority in edicom in months"""
        if self.edicom_init_date is None:
            print("error: edicom_init_date is not registered in this member.")
            return "?" 

        today = datetime.today()
        seniority = today - self.edicom_init_date
...
if __name__=="__main__":
    ...
    print("Antigüedad edicom", maria.get_edicom_seniority())
